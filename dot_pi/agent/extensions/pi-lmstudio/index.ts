...
/**
 * Translate LM Studio's reasoning capability metadata into Pi's thinking-level
 * map.  `reasoning: true` only tells Pi that a model can think; without a
 * thinkingLevelMap Pi assumes the standard levels through `high` are available.
 * That causes a model such as Qwen3.8, which supports `xhigh` but not `high`,
 * to be downgraded from Pi's configured `xhigh` level to `high`.
 *
 * LM Studio also exposes `on` for models that have a thinking toggle but no
 * effort levels. `on` is not a Pi level, so expose it as Pi's `low` only when
 * no actual effort level is available.
 */
type LMStudioReasoning = NonNullable<LMStudioModel["capabilities"]>["reasoning"];

function mapThinkingLevels(
  reasoning: LMStudioReasoning,
): ProviderModelConfig["thinkingLevelMap"] | undefined {
  if (!reasoning || typeof reasoning !== "object" || !("allowed_options" in reasoning)) {
    return undefined;
  }

  const allowed = new Set((reasoning as { allowed_options: string[] }).allowed_options);
  const effortLevels = ["minimal", "low", "medium", "high", "xhigh", "max"];
  const hasEffortLevel = effortLevels.some(level => allowed.has(level));

  return {
    off: allowed.has("off") ? "off" : null,
    minimal: allowed.has("minimal") ? "minimal" : null,
    low: allowed.has("low") ? "low" : !hasEffortLevel && allowed.has("on") ? "on" : null,
...
function mapModels(models: LMStudioModel[], providerName: string): ProviderModelConfig[] {
  return models.map(m => {
    ...
    return {
      ...
      thinkingLevelMap: mapThinkingLevels(reasoning),
      ...
      ...(reasoning !== undefined ? { compat: { supportsReasoningEffort: true } } : {}),
    };
  });
}
